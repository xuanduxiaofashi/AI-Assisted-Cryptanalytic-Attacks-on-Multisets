...
def new_transformer_model_from_64(input_shape=(120, 12), num_heads=4):
    inputs = tf.keras.Input(shape=input_shape)  
    x = inputs
    attn_output = layers.MultiHeadAttention(num_heads=num_heads, key_dim=12)(x, x)
    x = layers.Add()([x, attn_output])
    x = layers.LayerNormalization()(x)
    ff = layers.Dense(128, activation='relu')(x)
    ff = layers.Dense(12)(ff)
    x = layers.Add()([x, ff])
    x = layers.LayerNormalization()(x)
