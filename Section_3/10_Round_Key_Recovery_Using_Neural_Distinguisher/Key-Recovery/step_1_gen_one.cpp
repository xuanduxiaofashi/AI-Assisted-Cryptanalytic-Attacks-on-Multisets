...
std::string generateFileName(int train_num) {
    ...
    fileName << "cipher_txt_npy/" <<  "cipher" << "_8r_" << "2_" << train_num << "_key_11_12_"<< now << ".txt"; 
    return fileName.str();
}
...
int main()
{
    ...
    std::stringstream fileName;
    fileName << "ciper_txt_gen_16/" <<  "cipher" << "_8r_" << "gen_1"  << ".txt";  
    ...
    for (int i = 0 ; i < repeat_num; i ++)
    {
        ...
        {
            ciphertext_no_xor_process[p_k][2] = ciphertext_no_xor[p_k][6];
            ciphertext_no_xor_process[p_k][7] = ciphertext_no_xor[p_k][4] ^ ciphertext_no_xor[p_k][8] ^ ciphertext_no_xor[p_k][12] ;
            ciphertext_no_xor_process[p_k][14] = ciphertext_no_xor[p_k][1] ^ ciphertext_no_xor[p_k][13];
            
        }

        for(int k = 0; k < 16; k ++)
        {
            outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][2])[3] ) << " ";outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][2])[2] ) << " ";
            outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][2])[1] ) << " ";outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][2])[0] ) << " ";

            outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][7])[3] ) << " ";outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][7])[2] ) << " ";
            outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][7])[1] ) << " ";outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][7])[0] ) << " ";

            outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][14])[3] ) << " ";outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][14])[2] ) << " ";
            outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][14])[1] ) << " ";outfile << static_cast<int>(std::bitset<4>(ciphertext_no_xor_process[k][14])[0] ) << " ";
            outfile << endl;
        }
    }
