...
def gen_train_or_val_data(train_num, load_file_name):
    ...
    train_X_label_0 = np.random.choice([0, 1], size=(train_num_label_1  , 16 , 64))
    ...
    xor_result = np.bitwise_xor(train_X_label_0[:, i_indices, :], train_X_label_0[:, j_indices, :])

    train_X_label_0 = xor_result 
...
def train_one_model(train_num,  train_func, repeat_num, file_name_1, file_name_2):
    ...
    check = ModelCheckpoint(wdir + f'{model_name}_train_data_{train_num_record}_{t}'  + '.h5', monitor='val_loss', save_best_only=True)
