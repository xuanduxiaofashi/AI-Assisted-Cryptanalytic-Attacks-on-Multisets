...
import itertools
from neural_model import new_transformer_model_from_64

model_name = "new_transformer_model_from_64"

train_func = new_transformer_model_from_64
...
def gen_train_or_val_data(train_num,  load_file_name):
    ...
    train_X_label_0_linear =  train_X_label_0[:, :, [0, 1, 2, 3,  12, 13, 14, 15,  28, 29, 30, 31]]
    

    comb_indices = list(itertools.combinations(range(16), 2))

    i_indices = np.array([i for i, j in comb_indices])
    j_indices = np.array([j for i, j in comb_indices])


    xor_result = np.bitwise_xor(train_X_label_0_linear[:, i_indices, :], train_X_label_0_linear[:, j_indices, :])

    train_X_5_xor = np.bitwise_xor.reduce(train_X_label_0[:, :, [4, 5, 6, 7]],  axis=1).reshape(-1, 1, 4)
    train_X_9_xor = np.bitwise_xor.reduce(train_X_label_0[:, :, [16, 17, 18, 19, ]],  axis=1).reshape(-1, 1, 4)
    train_X_13_xor = np.bitwise_xor.reduce(train_X_label_0[:, :, [24, 25, 26, 27 ]],  axis=1).reshape(-1, 1, 4)
    train_X_6_xor = np.bitwise_xor.reduce(train_X_label_0[:, :, [8, 9, 10, 11]],  axis=1).reshape(-1, 1, 4)
    train_X_10_xor = np.bitwise_xor.reduce(train_X_label_0[:, :, [20, 21, 22, 23]],  axis=1).reshape(-1, 1, 4)
    train_X_14_xor = np.bitwise_xor.reduce(train_X_label_0[:, :, [28, 29, 30, 31 ]] ,  axis=1).reshape(-1, 1, 4)


    train_X_label_0_integral1 =   train_X_5_xor ^ train_X_9_xor ^  train_X_13_xor
    train_X_label_column_1 =  train_X_label_0_integral1[:, :, 0].reshape(-1, 1, 1)
    train_X_label_column_2 =  train_X_label_0_integral1[:, :, 2].reshape(-1, 1, 1)
    train_X_label_column_3 =    (train_X_label_0_integral1[:, :, 0] ^ train_X_label_0_integral1[:, :, 2]).reshape(-1, 1, 1)


    train_X_label_0_integral2 =   train_X_6_xor^ train_X_10_xor ^  train_X_14_xor
    train_X_label_column_4 =  train_X_label_0_integral2[:, :, 0].reshape(-1, 1, 1)
    train_X_label_column_5 =  train_X_label_0_integral2[:, :, 2].reshape(-1, 1, 1)
    train_X_label_column_6 =  (train_X_label_0_integral2[:, :, 0] ^ train_X_label_0_integral2[:, :, 2]).reshape(-1, 1, 1)


    zeros_colunn = np.zeros((train_num_label_1, 1, 1))
    train_X_last_column = np.concatenate(([train_X_label_column_1, train_X_label_column_2, train_X_label_column_3, 
                                          train_X_label_column_4, train_X_label_column_5, train_X_label_column_6, 
                                          zeros_colunn, zeros_colunn, zeros_colunn,
                                          zeros_colunn, zeros_colunn, zeros_colunn]),  axis = 2)
    
    train_X_label_0 = np.concatenate((xor_result, train_X_last_column ), axis=1)
